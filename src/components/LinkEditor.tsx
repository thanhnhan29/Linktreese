...
import { ecommerceService, PLACEHOLDER_IMAGE } from '@/features/bio-page/services/ecommerceService';
...
interface LinkEditorProps {
  ...
  profileImage: string;
  bio: string;
  onAddLink: (title: string, url: string, type?: string, platform?: string, data?: any, order?: number) => void;
  ...
  // Block operations
  onAddBlock: (type: 'ecommerce' | 'donate' | 'contact' | 'chat', title: string, data: Record<string, unknown>, order?: number) => Promise<void>;
  onUpdateBlock: (id: string, title?: string, data?: Record<string, unknown>) => Promise<void>;
  onDeleteBlock: (id: string) => Promise<void>;
  onToggleBlock: (id: string) => Promise<void>;
  // Unified move for cross-type ordering (direction or target index)
  onMoveUnified: (id: string, itemType: 'link' | 'block', directionOrTargetIndex: 'up' | 'down' | number) => Promise<void>;
}
...
export default function LinkEditor({
  ...
  const [draggedItem, setDraggedItem] = useState<{ id: string; type: 'link' | 'block' } | null>(null);
  ...
  const handleUnifiedDrop = async (targetId: string, targetType: 'link' | 'block') => {
    if (!draggedItem) return;
    if (draggedItem.id === targetId && draggedItem.type === targetType) return;

    const draggedIndex = unifiedItems.findIndex(
      u => (u.type === 'link' ? u.item.id : u.item.id) === draggedItem.id && u.type === draggedItem.type
    );
    const targetIndex = unifiedItems.findIndex(
      u => (u.type === 'link' ? u.item.id : u.item.id) === targetId && u.type === targetType
    );
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validation.isValid) {
      ...
      toast.error(validation.error || 'Invalid file format. Please upload .PNG, .JPG, or .JPEG only.');
      return;
    }
  ...
  const getBlockTypeFromLinkType = (lt: string): 'ecommerce' | 'donate' | 'contact' | 'chat' | null => {
  ...
  const validateAndBuildBlockData = async (
  ...
  ): Promise<{ title: string; data: Record<string, unknown>; message: string } | null> => {
    ...
    if (blockType === 'donate') {
      ...
      const methodName = donateMethod === 'vietqr' ? 'VietQR' : donateMethod === 'momo' ? 'Momo' : 'ZaloPay';
      return {
        title: donateTitle,
        data: {
          method: donateMethod,
    ...
    if (blockType === 'contact') {
      const validation = contactService.validateConfig({ title: contactTitle, receiverEmail: contactEmail });
      if (!validation.isValid) {
        setErrors({ [validation.field === 'title' ? 'contactTitle' : 'contactEmail']: validation.error });
        return null;
      }
      return {
        title: contactTitle,
    ...
    if (blockType === 'ecommerce') {
      ...
      if (!validation.isValid) {
        setErrors({ ecommerceUrl: validation.error || 'URL is not supported.' });
        return null;
      }
      const result = await ecommerceService.fetchProduct(ecommerceUrl);
      if (!result.success || !result.data) {
        setErrors({ ecommerceUrl: result.error || 'Unable to retrieve product information.' });
        return null;
      }
      const productData = { ...result.data, image: result.data.image || PLACEHOLDER_IMAGE, platform: validation.platform };
      return {
    ...
    if (blockType === 'chat') {
      ...
      if (!phoneRegex.test(chatPhoneNumber.trim())) {
        setErrors({ chatPhone: 'Invalid phone number. Please enter 10 digits starting with 0.' });
        return null;
      }
      const finalTitle = chatTitle.trim() || 'Chat on Zalo';
      return {
        title: finalTitle,
        data: { title: finalTitle, phoneNumber: chatPhoneNumber, message: chatMessage },
  ...
  const handleAddLink = async () => {
    ...
    if (linkType === 'social') {
      ...
      const validation = ecommerceService.validateUrl(ecommerceUrl);
      if (!validation.isValid) {
        newErrors.ecommerceUrl = validation.error || 'URL is not supported. Please use a link from Shopee or Lazada.';
        setErrors(newErrors);
        return;
      }
      ...
      try {
        ...
        if (!result.success || !result.data) {
          newErrors.ecommerceUrl = result.error || 'Unable to retrieve product information. Please check the URL or try again later.';
          setErrors(newErrors);
          setIsLoading(false);
          return;
        }
        ...
        toast.success(`Product "${productData.title}" from ${validation.platform?.charAt(0).toUpperCase()}${validation.platform?.slice(1)} added successfully!`);
        resetForm();
        setIsDialogOpen(false);
      } catch (error) {
        newErrors.ecommerceUrl = 'Unable to retrieve product information. Please check the URL or try again later.';
        setErrors(newErrors);
      } finally {
        setIsLoading(false);
      }
      ...
      if (donateMethod !== 'vietqr') {
        const linkValidation = donateService.validatePaymentLink(donatePaymentLink, donateMethod);
      ...
      try {
        ...
        toast.success(`Donate button "${donateTitle}" with ${methodName} payment added successfully!`);
        resetForm();
        setIsDialogOpen(false);
      } catch (error) {
      ...
      try {
        ...
        toast.success(`Contact form "${contactTitle}" (sends to ${contactEmail}) added successfully!`);
        resetForm();
        setIsDialogOpen(false);
      } catch (error) {
      ...
      toast.success(`Zalo chat button "${finalTitle}" (${chatPhoneNumber}) added successfully!`);
      resetForm();
      setIsDialogOpen(false);
    }
  };
  ...
  const handleUpdateLink = async () => {
    ...
    const currentPosition = unifiedItems.findIndex(u => u.type === 'link' && u.item.id === editingLink.id);
    const preserveOrder = editingLink.order ?? (currentPosition >= 0 ? unifiedItems[currentPosition].order : 0);
    ...
    if (isTypeChanged && newBlockType) {
      ...
      try {
        ...
        await onAddBlock(newBlockType, blockData.title, blockData.data, preserveOrder);
    ...
    if (linkType === 'social') {
      ...
      onUpdateLink(editingLink.id, linkData.title, linkData.url, 'social', detectedPlatform || undefined);
    ...
    setIsLoading(true);
    try {
      const blockData = await validateAndBuildBlockData(linkType as 'ecommerce' | 'donate' | 'contact' | 'chat');
      if (!blockData) {
        setIsLoading(false);
        return;
      }
      ...
      const url = linkType === 'chat' ? `https://zalo.me/${chatPhoneNumber}` : '';
      onUpdateLink(editingLink.id, blockData.title, url, linkType, undefined, blockData.data);
    ...
    if (block.type === 'ecommerce') {
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-[#676b5f]">{data?.price || 'N/A'} • {data?.platform || 'E-commerce'}</p>
          </div>
        </div>
      );
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-[#676b5f]">
              {data?.method === 'vietqr' ? 'VietQR' : data?.method === 'momo' ? 'Momo' : 'ZaloPay'} payment
            </p>
          </div>
        </div>
      );
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-black">{block.title}</p>
            <p className="text-sm text-[#676b5f]">Sends to: {data?.receiverEmail || 'N/A'}</p>
          </div>
        </div>
      );
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-black">{block.title}</p>
            <p className="text-sm text-[#676b5f]">{data?.phoneNumber || 'N/A'}</p>
          </div>
        </div>
      );
    }
  ...
  const handleEditBlock = (block: Block) => {
    ...
    if (block.type === 'donate') {
      ...
      setDetectedEcommercePlatform(data?.platform as 'shopee' | 'lazada' || null);
  ...
  const handleUpdateBlock = async () => {
    ...
    const currentPosition = unifiedItems.findIndex(u => u.type === 'block' && u.item.id === editingBlock.id);
    const preserveOrder = editingBlock.sortOrder ?? (currentPosition >= 0 ? unifiedItems[currentPosition].order : 0);
    ...
    if (isConvertingToLink) {
      ...
      try {
        ...
        onAddLink(linkData.title, linkData.url, 'social', detectedPlatform || undefined, undefined, preserveOrder);
    ...
    const blockData = await validateAndBuildBlockData(originalBlockType as 'ecommerce' | 'donate' | 'contact' | 'chat');
    if (!blockData) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open: boolean) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" aria-describedby="add-link-description">
          <DialogHeader>
            <DialogTitle>
              {editingBlock 
                ? 'Edit Block' 
                : editingLink 
                  ? (editingLink.type === 'social' || editingLink.type === 'regular' ? 'Edit Link' : 'Edit Block')
                  : (linkType === 'social' ? 'Add New Link' : 'Add New Block')}
            </DialogTitle>
          </DialogHeader>
          <p id="add-link-description" className="sr-only">
            {editingBlock || editingLink ? 'Edit your existing item' : 'Add a new item to your profile'}
          </p>
          ...
          <div className="space-y-4 mt-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.socialTitle) setErrors(prev => ({ ...prev, socialTitle: undefined }));
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.socialUrl) setErrors(prev => ({ ...prev, socialUrl: undefined }));
                    }}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.ecommerceUrl) setErrors(prev => ({ ...prev, ecommerceUrl: undefined }));
                    // Real-time detection
                    if (url) {
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.donateTitle) setErrors(prev => ({ ...prev, donateTitle: undefined }));
                    }}
                ...
                {donateMethod === 'vietqr' ? (
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        handleImageUpload(e);
                        if (errors.donateQR) setErrors(prev => ({ ...prev, donateQR: undefined }));
                      }}
                      className="hidden"
                    />
                    ...
                    {qrImagePreview ? (
                      ...
                      <button
                        ...
                        className={`w-full h-32 border-2 border-dashed rounded-lg flex flex-col items-center justify-center gap-2 hover:border-[#8129d9] hover:bg-[#f6f7f5] transition-all ${errors.donateQR ? 'border-red-400' : 'border-[#e0e2d9]'}`}
                      >
                        <Upload className="w-8 h-8 text-[#676b5f]" />
                        <p className="text-sm text-[#676b5f]">Click to upload QR image</p>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        // Clear error when editing
                        if (errors.donatePaymentLink) setErrors(prev => ({ ...prev, donatePaymentLink: undefined }));
                      ...
                      className={errors.donatePaymentLink ? 'border-red-500' : ''}
                    />
                    ...
                    {paymentLinkValid === true && !errors.donatePaymentLink && (
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.contactTitle) setErrors(prev => ({ ...prev, contactTitle: undefined }));
                    }}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.chatPhone) setErrors(prev => ({ ...prev, chatPhone: undefined }));
                    }}
            ...
            <Button
              onClick={editingBlock ? handleUpdateBlock : (editingLink ? handleUpdateLink : handleAddLink)}
            ...
            >
              {isLoading ? (
                ...
                editingBlock 
                  ? 'Update Block' 
                  : editingLink 
                    ? (linkType === 'social' ? 'Update Link' : 'Update Block')
                    : (linkType === 'social' ? 'Add Link' : 'Add Block')
              )}
            </Button>
          </div>
        </DialogContent>
      ...
      <div className="space-y-3">
        {unifiedItems.length === 0 ? (
          <div className="bg-white rounded-lg p-8 border border-[#e0e2d9] text-center">
            <p className="text-[#676b5f]">No links or blocks yet. Click the button above to get started!</p>
          </div>
        ) : (
          <>
                        ...
                        {unifiedItems.length > 1 && (
                          <div className="flex flex-col gap-1">
                            <button
                              ...
                              disabled={unifiedItems.findIndex(u => u.type === 'link' && u.item.id === link.id) === 0}
                              className="p-1 hover:bg-[#f6f7f5] rounded disabled:opacity-30 disabled:cursor-not-allowed"
                              title="Move up"
                            >
                            ...
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  await onMoveUnified(link.id, 'link', 'down');
                                } catch (error) {
                                  console.error('Error moving link down:', error);
                                  toast.error('Failed to move link');
                                }
                              }}
                              disabled={unifiedItems.findIndex(u => u.type === 'link' && u.item.id === link.id) === unifiedItems.length - 1}
                              className="p-1 hover:bg-[#f6f7f5] rounded disabled:opacity-30 disabled:cursor-not-allowed"
                              title="Move down"
                            >
                          ...
                          <div className="flex flex-col gap-1">
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  await onMoveUnified(block.id, 'block', 'up');
                                } catch (error) {
                                  console.error('Error moving block up:', error);
                                  toast.error('Failed to move block');
                                }
                              }}
                              disabled={unifiedItems.findIndex(u => u.type === 'block' && u.item.id === block.id) === 0}
                              className="p-1 hover:bg-[#f6f7f5] rounded disabled:opacity-30 disabled:cursor-not-allowed"
                              title="Move up"
                            >
                              <ChevronUp className="w-4 h-4 text-[#676b5f]" />
                            ...
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  await onMoveUnified(block.id, 'block', 'down');
                                } catch (error) {
                                  console.error('Error moving block down:', error);
                                  toast.error('Failed to move block');
                                }
                              }}
                              disabled={unifiedItems.findIndex(u => u.type === 'block' && u.item.id === block.id) === unifiedItems.length - 1}
                              className="p-1 hover:bg-[#f6f7f5] rounded disabled:opacity-30 disabled:cursor-not-allowed"
                              title="Move down"
                            >
                              <ChevronDown className="w-4 h-4 text-[#676b5f]" />
                        ...
                        <Switch
                          ...
                          onCheckedChange={async () => {
                            try {
                              ...
                              toast.error('Failed to toggle block visibility');
                            }
                          }}
                        />
          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="flex items-center gap-4">
              <Button
                onClick={() => {
                  if (linkToDelete) {
                    const typeName = linkToDelete.type === 'social' ? 'Social link' : 
                                     linkToDelete.type === 'chat' ? 'Zalo chat' : 
                                     linkToDelete.type === 'ecommerce' ? 'Product' : 
                                     linkToDelete.type === 'donate' ? 'Donate button' : 
                                     linkToDelete.type === 'contact' ? 'Contact form' : 'Link';
                    onDeleteLink(linkToDelete.id);
                    toast.success(`${typeName} "${linkToDelete.title}" has been deleted!`);
                  }
      ...
      <Dialog open={deleteBlockConfirmOpen} onOpenChange={setDeleteBlockConfirmOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto" aria-describedby="delete-block-description">
          <DialogHeader>
          ...
          <div className="space-y-4 mt-6">
            <p className="text-sm text-[#676b5f]">
              Are you sure you want to delete the block "{blockToDelete?.title}"?
            </p>
            ...
            <div className="flex items-center gap-4">
              <Button
                onClick={async () => {
                  if (blockToDelete) {
                    try {
                      const typeName = blockToDelete.type === 'ecommerce' ? 'Product' : 
                                       blockToDelete.type === 'donate' ? 'Donate button' : 
                                       blockToDelete.type === 'contact' ? 'Contact form' : 
                                       blockToDelete.type === 'chat' ? 'Zalo chat' : 'Block';
                      await onDeleteBlock(blockToDelete.id);
                      toast.success(`${typeName} "${blockToDelete.title}" has been deleted!`);
                    } catch (error) {
